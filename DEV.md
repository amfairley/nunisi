...
**user_profile**: The user profile app was named due to the pre-existing Django default apps of user and profile to avoid any complication with the code. It holds the code for the user profile data including the models for the UserProfile and Trips, the form for updating the user profile information, and a signal to create a user profile for users when the sign up or update the profile if they make changes. There are links on the user_profile.html that link to allauth functionality for changing/adding email addresses or changing the user password. The trips.html page shows the user their previous and upcoming trips, and the trips_superuser.html page shows the admin a full list of all trips made. 
